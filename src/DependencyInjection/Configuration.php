                                    ...
                                    ->arrayNode('connection_settings')
                                        ...
                                        ->arrayPrototype()
                                            ->children()
                                                ->scalarNode('connect_pipeline')->defaultNull()->end()
                                                ->scalarNode('query_pipeline')->defaultNull()->end()
                                                ->scalarNode('connect_lane')->defaultNull()->end()
                                                ->scalarNode('query_lane')->defaultNull()->end()
                                            ->end()
                                        ->end()
                                        ->defaultValue([])
                                    ->end()
                                    ->arrayNode('routing_lanes')
                                        ->addDefaultsIfNotSet()
                                        ->children()
                                            ->scalarNode('parent_pipeline')->defaultNull()->end()
                                            ->arrayNode('parent_lane_map')
                                                ->useAttributeAsKey('pattern')
                                                ->scalarPrototype()->end()
                                                ->defaultValue([])
                                            ->end()
                                            ->scalarNode('child_pipeline')->defaultNull()->end()
                                            ->arrayNode('child_lane_map')
                                                ->useAttributeAsKey('pattern')
                                                ->scalarPrototype()->end()
                                                ->defaultValue([])
                                            ->end()
